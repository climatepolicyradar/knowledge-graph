...
def main(
    ...
    classifier_classes: list[Classifier] = [
        KeywordClassifier,
        RulesBasedClassifier,
        StemmedKeywordClassifier,  # type: ignore
    ...
    for classifier_class in classifier_classes:
        classifier: Classifier = classifier_class(concept)  # type: ignore
        classifier.fit()
        console.log(
            f"✅ Trained a {classifier.name} classifier for {concept.wikibase_id}"
        )
        classifier_path = (
            classifier_dir / str(concept.wikibase_id) / f"{classifier.name}.pickle"
        )
