...
def main(
        ...
        for wikibase_id in group:
            ...
            for statement in flat_statements:
                ...
                if item_id is not None and item_id in group:
                    console.print(
                        f"Removing claim {statement['id']} from item {wikibase_id}"
                    )
                    ...
                    if remove_claim_response.status_code == 200:
                        console.print(
                            f"Removed claim {statement['id']} from item {wikibase_id}",
                            style="green",
                        )
                    if remove_claim_response.status_code != 200:
                        console.print(
            ...
            for wikibase_id in ids_to_merge:
                console.print(f"Merging item {wikibase_id} into item {target_item}")
                ...
                if merge_response.status_code == 200:
                    console.print(
                        f"Merged item {wikibase_id} into item {target_item}",
                        style="green",
                    )
                if merge_response.status_code != 200:
                    console.print(
    ...
    console.print("Finished merging concepts", style="green")
