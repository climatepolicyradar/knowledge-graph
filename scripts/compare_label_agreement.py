...
def calculate_agreement_metrics(
    ...
    """
    assert len(ensemble_labels) == len(best_model_labels), (
        "Label lists must be same length"
    )
...
async def async_main(
    ...
    ensemble_passages = load_labelled_passages_from_wandb(
        wandb_path=ensemble_predictions_path
    )
    ...
    ensemble_labels = [
        len(passage.spans) > 0
        and passage.spans[0].prediction_probability is not None
        and passage.spans[0].prediction_probability > 0.5
    ...
    best_model_predictions = best_model_classifier.predict(
        passage_texts, batch_size=50, show_progress=True
    )
    ...
    summary_table.add_row("Agreement Rate", f"{metrics['agreement_rate']:.1%}")
