...
async def main() -> None:
    ...
    artifacts: list[Artifact] = await client.read_artifacts(
        flow_run_filter=FlowRunFilter(
            name=FlowRunFilterName(
                any_=[f"deprecating-static-classifiers-{n}" for n in range(1, 17)],
            ),
        )
    )
    ...
    with open("post_run_document_ids_missing.txt", "w") as f:
        for doc_id in document_ids:
            f.write(f"{doc_id}\n")
    
    with open("post_run_other_errors.txt", "w") as f:
        for error in all_other_errors:
            doc_id = error.get("Failed document ID")
            exception = error.get("Exception")
            f.write(f"{doc_id}: {exception}\n")
    
    print(f"Wrote {len(document_ids)} document IDs to post_run_document_ids_missing.txt")
    print(f"Wrote {len(all_other_errors)} other errors to post_run_other_errors.txt")
