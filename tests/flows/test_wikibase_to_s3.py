...
@pytest.mark.asyncio
async def test_wikibase_to_s3__trigger_indexing_called(
    MockedWikibaseSession,
    mock_cdn_bucket,
    mock_prefect_slack_webhook,
    test_wikibase_to_s3_config,
    ...
    # Mock the trigger_indexing function to track if it's called
    trigger_indexing_called = False

    async def mock_trigger_indexing(extras_in_s3, _config):
        nonlocal trigger_indexing_called
        trigger_indexing_called = True
        assert extra_concept_id in extras_in_s3
        return None

    monkeypatch.setattr("flows.wikibase_to_s3.trigger_indexing", mock_trigger_indexing)

    # Run the flow
    await wikibase_to_s3.fn(config=test_wikibase_to_s3_config)

    # Verify trigger_indexing was called
    assert trigger_indexing_called, "trigger_indexing should have been called"
