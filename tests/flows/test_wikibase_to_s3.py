...
async def test_upload_to_s3(
    ...
    await upload_to_s3(
        config=test_wikibase_to_s3_config,
...
async def test_delete_from_s3(
    ...
    await delete_from_s3(
        config=test_wikibase_to_s3_config,
        concept_id=mock_concept.wikibase_id,
        s3_client=mock_s3_async_client,
    )
...
async def test_list_s3_concepts(
    ...
    results = await list_s3_concepts(
        config=test_wikibase_to_s3_config, s3_client=mock_s3_async_client
    )
    assert set(results) == {
        "Q10",
        "Q20",
...
async def test_wikibase_to_s3__extra_concept_removed(
    ...
    s3_concepts_before = await list_s3_concepts(
        config=test_wikibase_to_s3_config, s3_client=mock_s3_async_client
    )
    ...
    s3_concepts_after = await list_s3_concepts(
        config=test_wikibase_to_s3_config, s3_client=mock_s3_async_client
    )
