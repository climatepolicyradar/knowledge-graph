...
@pytest.mark.asyncio
async def test_store_index_metadata(
    test_config,
    mock_async_bucket,
    mock_s3_async_client,
    snapshot,
):
    """Test that store_index_metadata correctly builds S3 URI and stores metadata."""
    ...
    expected_key = os.path.join(
        test_config.index_inference_results_prefix,
        mock_run_output_id,
        "index_metadata.json",
    )
    ...
    metadata = IndexMetadata.model_validate(metadata_dict)
