...
def get_vespa_search_adapter() -> VespaSearchAdapter:
    """
    Get a Vespa search adapter.

    It uses certs fetched, and then, saved to disk, if none were
    provided.
    """
    # We want the directory used for the `VespaSearchAdapter` to be
    # automatically cleaned up.
    #
    # To do this, we rely on the `tempfile.TemporaryDirectory`'s
    # behaviour, for when the object is destroyed [1].
    #
    # [1] > On completion of the context or destruction of the temporary directory object, the newly created temporary directory and all its contents are removed from the filesystem.
...
async def partial_update_text_block(
    ...
    response: VespaResponse = await vespa_connection_pool.update_data(  # pyright: ignore[reportOptionalMemberAccess]
        ...
        for batch_num, batch in enumerate(tasks_batches, start=1):
            ...
            partial_update_tasks = [
                partial_update_text_block(
                    text_block=(
                        # It's a little awkward, but this is our current data structures
                        text_blocks[text_block_id][0],
                        text_blocks[text_block_id][1],
                    ),
                    concepts=concepts,
                    vespa_connection_pool=vespa_connection_pool,
