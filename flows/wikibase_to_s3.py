...
async def upload_to_s3(config: Config, concept: Concept, s3_client: S3Client) -> None:
...
async def delete_from_s3(config: Config, concept_id: str, s3_client: S3Client) -> None:
    """Delete an individual concept from S3"""
    filename = f"{concept_id}.json"
    key = os.path.join(config.s3_prefix, filename)

    _ = await s3_client.delete_object(
        Bucket=config.get_cdn_bucket_name(),
        Key=key,
    )


async def list_s3_concepts(config: Config, s3_client: S3Client) -> list[str]:
...
async def delete_extra_concepts_from_s3(
    extras_in_s3: list[str], config: Config, s3_client: S3Client
) -> list[str]:
    ...
    for i, concept_id in enumerate(extras_in_s3, start=1):
        ...
        try:
            await delete_from_s3(config, concept_id, s3_client)
        except Exception as e:
            logger.error(f"Failed to delete concept #{i}: {concept_id}, error: {e}")
    return failures


async def upload_to_local(concept: Concept) -> None:
    """Write an individual concept to a local file as JSON."""
    filename = f"{concept.wikibase_id}.json"
    local_output_dir: str = "concepts_local"
    output_dir = local_output_dir
    os.makedirs(output_dir, exist_ok=True)
    file_path = os.path.join(output_dir, filename)
    with open(file_path, "wb") as f:
        f.write(concept.model_dump_json().encode())
    ...
    async with session.client("s3") as s3_client:
        for i, wikibase_id in enumerate(wikibase_ids, start=start):
            ...
            if i == start or i % config.logging_interval == 0:
                logger.info(f"Getting concepts #{i}..#{next_interval}")
            try:
                concept = await wikibase.get_concept_async(
                    wikibase_id, include_recursive_subconcept_of=True
                )
                await upload_to_s3(config, concept, s3_client)
            except Exception as e:
                logger.error(
                    f"Failed to upload concept #{i}: {wikibase_id}, error: {e}"
                )
                failed_wikibase_ids_uploads.append(wikibase_id)

        # Identify leftovers from prior runs
        s3_concepts = await list_s3_concepts(config, s3_client)
