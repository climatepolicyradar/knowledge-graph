"""
Reusable logic for refreshing classifier spec files from W&B.

These helpers depend on `flows.classifier_specs` types (`ClassifierSpec`,
`DontRunOnEnum`, …), so they live in `flows/` rather than
`knowledge_graph/operations/` (operations must not import from flows). The
`scripts/update_classifier_spec.py` CLI is a thin Typer wrapper around
...
def refresh_all_available_classifiers(aws_envs: list[AwsEnv] | None = None) -> None:
    ...
    log.info(f"Running for AWS environments: {[aws_env.name for aws_env in aws_envs]}")
    api = wandb.Api()
