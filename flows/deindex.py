...
async def update_s3_with_latest_concepts_counts(
    ...
    non_zero_counter = Counter()

    for k, v in concepts_counts.items():
        if v != 0:
            non_zero_counter.update({k: v})
        else:
            zero_counter.update({k: v})
    ...
    if len(non_zero_counter) == 0:
        logger.info(f"successfully updated all concepts: {zero_counter}")
    ...
    # We didn't succeed with all, so write the concepts counts still
        ...
        logger.info(
            f"only updated some concepts. Zero: {zero_counter}, non-zero: {non_zero_counter}"
        )
        update_s3_with_some_successes(
            document_object_uri=document_importer[1],
            concepts_counts_filtered=non_zero_counter,
            document_labelled_passages=document_labelled_passages,
            cache_bucket=cache_bucket,
            concepts_counts_prefix=concepts_counts_prefix,
        )
