    ...
    with cm:
        ...
        for batch_num, batch in enumerate(batches, start=1):
            ...
            for i, result in enumerate(results):
                ...
                concepts_counts.subtract(concepts_models)
        ...
        # If after filtering out, there's on concepts, that means we
        ...
        if len(concepts_counts_filtered) == 0:
            ...
            try:
                ...
                logger.error(f"failed to remove concepts counts from S3: {str(e)}")
        # We didn't succeed, so write the concepts counts still
        else:
            ...
            try:
                concept_ids_to_keep: list[WikibaseID] = [
                    concept_model.wikibase_id for concept_model in concepts_counts
                ]
                ...
                logger.error(
                    f"failed to write updated labelled passages to S3: {str(e)}"
                )
