...
def fn_is_async(fn: Callable[..., Any] | Flow[P, R]) -> bool:
    ...
    if isinstance(fn, Flow):
        if isinstance(fn.fn, CoiledFunction):
            # The coiled decorator interferes with the flow async analysis
            # so we can rely on fn.async
            return inspect.iscoroutinefunction(fn.function)  # type: ignore[reportFunctionMemberAccess]
