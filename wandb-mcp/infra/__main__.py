...
# The SSM parameter is named "WANDB_API_KEY"; the ARN form requires a leading slash.
ssm_wandb_api_key_arn = get_ssm_parameter_arn("/WANDB_API_KEY")
...
dockerfile_path = (root_dir / "wandb-mcp" / "Dockerfile").resolve()
...
ssm_policy = aws.iam.Policy(
    ...
    description="Policy to allow reading the W&B API key from SSM Parameter Store",
    policy=pulumi.Output.all(
        ssm_wandb_api_key_arn,
    ).apply(
            ...
            [
                {
                    ...
                    "secrets": [
                        {"name": "WANDB_API_KEY", "valueFrom": args[1]},
                    ],
                    "logConfiguration": {
                        "logDriver": "awslogs",
                        "options": {
...
pulumi.export("url", "https://wandb-mcp.labs.climatepolicyradar.org")
pulumi.export("mcp_url", "https://wandb-mcp.labs.climatepolicyradar.org/mcp")
pulumi.export(
    "health_check_url",
    "https://wandb-mcp.labs.climatepolicyradar.org/health",
)
