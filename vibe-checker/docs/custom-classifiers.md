...
Assuming you have a classifier that implements `.predict(texts) -> list[list[Span]]`, you should be able to adapt the example in [`custom_concept_demo.py`](custom_concept_demo.py) to get started. It should walk you through the structures that you eventually need to push to S3.
...
The webapp uses the following metadata fields for filtering, so they need to be present on each one of the labelled passages in `predictions.jsonl`. These should all be stringified values, matching what the standard flow produces from the passages dataset.
